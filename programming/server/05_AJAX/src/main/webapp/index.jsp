...
<body>
	...
	<h4>1) POST 방식으로 서버에 데이터 전송 및 응답</h4>
		...
		$(document).ready(() => {
			$("#btn3").on("click", () => {
				...
				$.ajax({
					type: "get",
					url: "jqAjax2.do",
					dataType: "json", //응답 데이터 형식(미 작성시 자동으로 음답 데이터 형식을 판단하여 지정한다.)
					data: {
						userNo
					},
					success: function(obj){
						...
						if(obj != null){
							result = 'no : ' + obj.no + ", name : " +  obj.age + ", gender : " + obj.gender;
						}else{
							result = "사용자 정보가 없습니다.";
			...
			$("#btn4").on("click", () => {
				...
				$.ajax({
					...
					success: (list) => {
						let result = "";
						$.each(list, (i)=> {
							result += "no : " + list[i].no + ", name : " + list[i].name + "\n";
						})
